...
import { getCacheOrSet, CACHE_PREFIXES, DEFAULT_TTL } from '@/lib/cache'
...
export async function GET(request) {
  try {
    ...
    const cacheKey = `${CACHE_PREFIXES.REPORTS}${period}_${paymentMethodId || 'all'}_${startDate || ''}_${endDate || ''}`
    
    // Use cache or generate fresh report
    const reportData = await getCacheOrSet(
      cacheKey,
      async () => {
